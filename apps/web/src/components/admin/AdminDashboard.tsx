...
const ICON_PATHS: Record<string, React.ReactNode> = {
  dishes: (
    <>
      <path d="M3 11h14" />
      <path d="M10 4a6 6 0 0 1 6 6H4a6 6 0 0 1 6-6Z" />
      <path d="M6 16h8" />
    </>
  ),
  families: (
  ...
  source: (
    <>
      <path d="M5 4h8a2 2 0 0 1 2 2v10H7a2 2 0 0 1-2-2z" />
      <path d="M8 8h4M8 11h4" />
    </>
  ),
  pin: (
    <>
      <path d="M10 2a5 5 0 0 0-5 5c0 3.6 5 11 5 11s5-7.4 5-11a5 5 0 0 0-5-5Z" />
...
export function AdminDashboard() {
  ...
  async function load() {
    ...
    if (statsRes.ok && statsRes.data) {
      const recentRes = await fetchJson<AdminDishListResponse>(
        "/api/admin/dishes?limit=8",
      );
    ...
    <div className="adm-dash">
      <header className="a-pagehead">
        ...
        <p className="a-pagehead__lede">
          Manage dishes, taxonomy and provenance across the Gustale culinary
          archive — the internal editing surface for the encyclopedia.
        </p>
      </header>
      ...
      {phase === "error" && (
        <div className="a-empty a-empty--error">
          ...
          <p>{error}</p>
          <button className="a-btn a-btn--accent" onClick={() => void load()}>
            Try again
          </button>
        </div>
      )}
        ...
        <>
          {degraded && (
            <div className="adm-note" role="status">
              <span className="adm-note__icon">
                <Icon name="alert" size={17} />
              </span>
              <span>
                <b>Showing partial data.</b> The aggregate stats service isn’t
                available on this deploy yet, so photo and source coverage are
                hidden. Counts below are derived from the dish and taxonomy
                lists.
              </span>
            </div>
          )}
          ...
          <div className="adm-kpis">
            ...
            <Kpi
              label="Published"
              num={stats.dishes.published}
              sub={<>live on the site</>}
            />
            <Kpi
              label="In draft"
              num={stats.dishes.draft}
              sub={<>awaiting review</>}
            />
            <Kpi
              label="Archived"
              num={stats.dishes.archived}
              sub={<>hidden entries</>}
            />
          </div>
          ...
          <div className="adm-actions">
            ...
            <Action
              ...
              sub={`${stats.dishes.draft} in draft`}
            />
            <Action
              href="/families"
              icon="families"
              title="Manage families"
              sub="Dish-type groups"
            />
            <Action
              href="/lineages"
              icon="lineages"
              title="Manage lineages"
              sub="Preparation methods"
            />
          </div>
          ...
          <div className="a-section-rule">
            <h2>Manage</h2>
            <span className="a-rule-count">
              {stats.dishes.total} dishes · {stats.taxonomy.categories}{" "}
              categories
            </span>
          </div>
          <div className="a-hub-grid">
            ...
            <HubTile
              icon="ingredients"
              title="Ingredients"
              count={stats.taxonomy.ingredients}
              unit="names"
              soon
            ...
            <HubTile
              icon="categories"
              title="Categories"
              count={stats.taxonomy.categories}
              unit="terms"
              soon
          ...
          <div className="adm-health">
            <HealthCard
              name="Missing photos"
              icon="photo"
              num={stats.health.missingPhoto}
            />
            <HealthCard
              name="Missing descriptions"
              icon="doc"
              num={stats.health.missingDescription}
            />
            <HealthCard
              name="Missing sources"
              icon="source"
              num={stats.health.missingSources}
            />
            <HealthCard
              name="No origin set"
              icon="pin"
              num={stats.health.missingOrigin}
            />
          </div>
          ...
          <div className="a-section-rule">
            ...
            <span className="a-rule-count">
              last {recent.length} {recent.length === 1 ? "change" : "changes"}
            </span>
          </div>
          {recent.length === 0 ? (
            <div className="a-empty">
              <div className="a-empty__icon">
                <Icon name="dishes" size={24} />
              </div>
              <h3>No dishes yet</h3>
              <p>Create your first dish entry to start building the archive.</p>
              <a className="a-btn a-btn--accent" href={ADD_DISH_HREF}>
                Add a dish
              </a>
            </div>
          ) : (
            <div className="a-recent">
              {recent.map((d) => (
                <a
                  key={d.slug}
                  className="a-recent__row"
                  href={`/admin/dishes/${d.slug}`}
                >
                  <span className="a-recent__name">{d.canonicalName}</span>
                  <span className="a-recent__origin">
                    {d.originName ?? "—"}
                  </span>
                  <StatusBadge status={d.status} />
                  <span className="a-recent__date">
                    {formatRelative(d.updatedAt)}
                  </span>
                  <span className="a-recent__chev">
...
function Action({
  ...
  return (
    <a
      className={`adm-action${primary ? " adm-action--primary" : ""}`}
      href={href}
    >
      <span className="adm-action__icon">
...
function HubTile({
  ...
  const inner = (
    <>
      <div className="a-hub-tile__top">
        <span className="a-hub-tile__icon">
          <Icon name={icon} size={20} />
        </span>
        {soon ? (
          <span className="a-soon-badge">Soon</span>
        ) : (
          <span className="a-hub-tile__count">
            {count.toLocaleString()} <i>{unit}</i>
          </span>
        )}
      </div>
      <h3>{title}</h3>
  ...
  if (soon || !href) {
    return (
      <div className="a-hub-tile a-hub-tile--soon" aria-disabled="true">
        {inner}
      </div>
    );
  }
  return (
    <a className="a-hub-tile" href={href}>
      {inner}
    </a>
  );
}
...
function HealthCard({
  ...
  const body = (
    <>
      <div className="adm-health__top">
        ...
        <span
          style={{ marginLeft: "auto", color: "var(--sub)", opacity: 0.55 }}
        >
  ...
  if (!unknown && !clear) {
    return (
      <a className="adm-health__card" href="/admin/dishes">
        {body}
      </a>
    );
  }
    ...
    <div aria-busy="true" aria-label="Loading dashboard">
      <div className="adm-kpis is-loading">
        {Array.from({ length: 4 }).map((_, i) => (
          <div key={i} className="a-skel-block" />
        ))}
      </div>
      <div className="a-section-rule">
        <h2 style={{ opacity: 0.4 }}>Loading…</h2>
      </div>
      <div className="a-hub-grid">
