...
export function EditDishForm({ slug }: EditDishFormProps) {
  ...
  const [lastDiff, setLastDiff] = useState<Record<
    string,
    ...
    void (async () => {
      try {
        ...
        setOriginDateEarliest(
          data.dish.originDateEarliest != null
            ? String(data.dish.originDateEarliest)
            : "",
        );
        setOriginDateLatest(
          data.dish.originDateLatest != null
            ? String(data.dish.originDateLatest)
            : "",
        );
        ...
        if (err instanceof ApiError && err.status === 404) {
          ...
          setLoadError(
            err instanceof Error ? err.message : "Could not load dish.",
          );
        }
      } finally {
  ...
  async function handleAddTag(): Promise<void> {
    ...
    try {
      const res = await fetch(
        `${import.meta.env.PUBLIC_API_BASE ?? ""}/api/tags`,
        {
          method: "POST",
          credentials: "include",
          headers: { "Content-Type": "application/json" },
  ...
  if (!user) {
    return (
      <div className="rounded-md border border-amber-200 bg-amber-50 p-6 text-sm text-amber-900">
        <p>
          <strong>Sign in required.</strong> Editing a dish requires an account.
        </p>
        <a
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div className="rounded-md border border-slate-200 bg-white p-6 shadow-sm">
        ...
        <p className="mt-1 text-sm text-slate-500">
          Categories, tags, and regional variants — how this dish is found and
          grouped.
        </p>
        ...
        <div className="mt-5">
          ...
          <p className="text-xs text-slate-500">
            Check all that apply; pick one as primary.
          </p>
                ...
                return (
                  <div key={parent.id} className="py-1">
                    {group.map((cat) => {
                      ...
                      const isPrimary = selectedCategories.find(
                        (c) => c.categoryId === cat.id,
                      )?.isPrimary;
                      return (
                        <div
