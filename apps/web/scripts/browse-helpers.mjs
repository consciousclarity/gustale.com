...
export function countryMatchesExact(originName, country) {
  ...
  return (
    (originName ?? "").toLowerCase() === String(country).trim().toLowerCase()
  );
}
  ...
  return [...groups.entries()]
    .map(([slug, list]) => ({
      ...
      dishNames: list
        ...
        .slice(0, 8),
      sampleOrigins: [
        ...new Set(list.map((d) => d.originName).filter(Boolean)),
      ].slice(0, 4),
    }))
...
export function matchesLineageFilters(lin, state) {
  ...
  if (state.technique) {
    if (!(lin.techniques ?? []).map(kebab).includes(kebab(state.technique)))
      return false;
  }
  if (state.force) {
    if (!(lin.historicalForces ?? []).map(kebab).includes(kebab(state.force)))
      return false;
  }
