...
describe('result filtering', () => {
  const dishes = [
    ...
    { id: '3', slug: 'burger', canonicalName: 'Hamburger', originName: 'United States', familySlug: 'sandwich', familyName: 'Sandwich', shortDescription: 'Patton' },
  ];
  ...
  it('uses exact case-insensitive country matching', () => {
    ...
    assert.equal(applyBrowseFilter(dishes, parseBrowseState({ country: 'United' })).length, 0);
    assert.equal(applyBrowseFilter(dishes, parseBrowseState({ country: 'united states' })).length, 1);
  });
});
...
describe('pagination append/dedupe/offset', () => {
  ...
  it('complete append/dedupe across pages reaches unique total', () => {
    const pageSize = 24;
    const all = Array.from({ length: 60 }, (_, i) => ({ id: `id-${i}`, slug: `slug-${i}` }));
    const pages = [
      all.slice(0, 24),
      all.slice(24, 48),
      all.slice(48, 60),
...
describe('history restoration plans', () => {
  ...
  it('extends for Forward restoration', () => {
    assert.deepEqual(planHistoryRestore(3, 1), { action: 'extend', fromPage: 1, toPage: 3 });
  ...
  it('shared ?page=N reconstruction uses extend from loaded 0/1', () => {
    ...
    assert.deepEqual(planHistoryRestore(3, 0), { action: 'extend', fromPage: 0, toPage: 3 });
