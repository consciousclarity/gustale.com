...
import { bucketsExist, getClient } from "../src/lib/minio.js";
...
describe("POST /api/media/upload — auth", () => {
  it("returns 401 when no session", async () => {
    ...
    const res = await app.inject({
      ...
      ...buildMultipart(
        {},
        { name: "t.jpg", mimeType: "image/jpeg", content: TINY_JPEG },
      ),
    });
...
describe("POST /api/media/upload — validation", () => {
  ...
  it("returns 400 when no file part is present", async () => {
    ...
    const res = await app.inject({
      ...
      url: "/api/media/upload",
      ...buildMultipart(
        { altText: "no file here" },
        { name: "x.jpg", mimeType: "image/jpeg", content: Buffer.alloc(0) },
      ),
    });
...
describe("POST /api/media/upload — happy path (requires MinIO)", () => {
  it.skipIf(!minioReachable)(
    "accepts JPEG, writes media row, returns 201",
    async () => {
      asContributor();
      const res = await app.inject({
        method: "POST",
...
describe("POST /api/dishes/:slug/media — attach + detach", () => {
  it.skipIf(!minioReachable)(
    "rejects 404 media_not_found for missing media",
    async () => {
      asContributor();
      const res = await app.inject({
        method: "POST",
  ...
  it.skipIf(!minioReachable)(
    "full happy path: upload → attach → detach",
    async () => {
      asContributor();
      // 1. Upload
