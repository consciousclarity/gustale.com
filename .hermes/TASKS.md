...
`apps/api`, `apps/web`, or `packages/db` even define a `lint`
script. No ESLint, Biome, or Prettier is installed anywhere in
the repo, and no config file exists. The green checkmark on every
PR's lint job means "the echo command printed the string
'lint ui'" — nothing else.

`continue-on-error: true` on the `Lint` step (line 69) hides the
absence of any real lint from anyone reading the workflow. Even
if a linter is configured tomorrow, that flag means the first
PR that introduces lint errors will fail visibly for the wrong
reason (the debt problem, not the regression problem), and the
natural reaction will be to keep `continue-on-error: true`
permanently.
...
2. `continue-on-error: true` is removed from the `Lint` step.
3. At least one PR has shipped with lint catching a real issue
   (proves the gate is wired correctly).
4. This Backlog entry is deleted; the result moves to the "Done"
   list.
